 ...
 */
export const checkAndCreateRewardRedemption = async (userId: string): Promise<void> => {
  try {
    ...
    if (points >= 2000) {
      ...
      const latestTier2 = await RewardRedemption.findOne({
        userId: user._id,
        rewardTier: 2
      }).sort({ createdAt: -1 });
    ...
    if (points >= 1000 && points < 2000) {
      ...
      const latestTier1 = await RewardRedemption.findOne({
        userId: user._id,
        rewardTier: 1
      }).sort({ createdAt: -1 });
